import { runWslProcess, WslGuestEnvironmentUnavailableError } from '../../../wsl/wsl-runner'
...
async function probeWslHerdrPath(distro: string): Promise<string> {
  try {
    ...
    const unavailable =
      error instanceof WslGuestEnvironmentUnavailableError
        ? `WSL login PATH is unverifiable for distro ${distro}`
        : `Herdr is not on the WSL login PATH for distro ${distro}`
    throw new HerdrRuntimeError('herdr_unavailable', unavailable)
  }
}
