...
function projectHerdrActivation(
  store: Store,
  terminalSettings: HorcaTerminalSettingsSource,
  ...
  return async (
    ...
    if (!projectWantsHerdr(project.id, terminalSettings, hostId)) {
      return null
    }
    ...
    return {
      activateHerdr: () => {
        commitHerdrHostActivation(terminalSettings, project.id, hostId)
      },
...
export function createLocalHerdrPtyTargetResolver(
  store: Store,
  terminalSettings: HorcaTerminalSettingsSource = createHorcaTerminalSettingsSource(store)
): HerdrPtyTargetResolver {
  return projectHerdrActivation(store, terminalSettings, LOCAL_EXECUTION_HOST_ID)
}
...
export function createHerdrPtyTargetResolver(
  ...
  return projectHerdrActivation(store, terminalSettings, hostId)
}
